...
var _ApiV = _interopRequireWildcard(require("./ApiV2"));
...
class UserManagerInstance {
  constructor() {
    _defineProperty(this, "loginServer", null);

    _defineProperty(this, "refreshSessionThreshold", 60 * 60);
  ...
  async _readUserData() {
    let auth = await _UserSettings.default.getAsync('auth', {});
    ...
    if ((0, _isEmpty.default)(auth)) {
      ...
      auth = await _UserSettings.default.getAsync('auth', {});
    }
  ...
  async getCurrentUserAsync() {
    ...
    try {
      ...
      let {
        currentConnection,
        sessionSecret
      } = await this._readUserData(); // No session, no current user. Need to login
  ...
  async _getProfileAsync({
    ...
    await _UserSettings.default.mergeAsync({
      auth: {
        userId: user.userId,
...
function _parseAuth0Profile(rawProfile) {
  ...
  return Object.keys(rawProfile).reduce((p, key) => {
    p[_lodash.default.camelCase(key)] = _parseAuth0Profile(rawProfile[key]);
    return p;
  }, {});
}
...
function _prepareAuth0Profile(niceProfile) {
  ...
  return Object.keys(niceProfile).reduce((p, key) => {
    p[_lodash.default.snakeCase(key)] = _prepareAuth0Profile(niceProfile[key]);
    return p;
  }, {});
}
