...
const optimizeImageAsync = async (image, newName, quality) => {
  _Logger.default.global.info(`Optimizing ${image}`);

  _fs.default.copyFileSync(image, newName); // Extract the format and compress


  const buffer = await (0, _sharp.default)(image).toBuffer();
  const {
    format
  } = await (0, _sharp.default)(buffer).metadata();

  if (format === 'jpeg') {
    await (0, _sharp.default)(newName).jpeg({
      quality
    }).toFile(image).catch(err => _Logger.default.global.error(err));
  } else {
    await (0, _sharp.default)(newName).png({
      quality
    }).toFile(image).catch(err => _Logger.default.global.error(err));
  }
};
