...
function _renderUnversionedReactNativeDependency(options, sdkVersion) {
  let sdkMajorVersion = (0, _ExponentTools.parseSdkMajorVersion)(sdkVersion);
...
async function _concatTemplateFilesInDirectoryAsync(directory, filterFn) {
  let templateFilenames = (await (0, _globPromise.default)(_path.default.join(directory, '*.rb'))).sort();
  let filteredTemplateFilenames = filterFn ? templateFilenames.filter(filterFn) : templateFilenames;
  let templateStrings = []; // perform this in series in order to get deterministic output
  ...
  for (let fileIdx = 0, nFiles = filteredTemplateFilenames.length; fileIdx < nFiles; fileIdx++) {
    ...
    let templateString = await _fsExtra.default.readFile(filename, 'utf8');
...
async function _renderPodDependenciesAsync(dependenciesConfigPath, options) {
  ...
  let depsStrings = dependencies.map(dependency => `  ${type} '${dependency.name}', '${dependency.version}'${noWarningsFlag}`);
...
async function renderPodfileAsync(pathToTemplate, pathToOutput, moreSubstitutions, shellAppSdkVersion, sdkVersion = 'UNVERSIONED') {
  ...
  let versionedDependencies = await _renderVersionedReactNativeDependenciesAsync(templatesDirectory, versionedRnPath, rnExpoSubspecs, shellAppSdkVersion);
  let versionedPostinstalls = await _renderVersionedReactNativePostinstallsAsync(templatesDirectory, shellAppSdkVersion);
  let podDependencies = await _renderPodDependenciesAsync(_path.default.join(templatesDirectory, 'dependencies.json'), {
    isPodfile: true
  });
