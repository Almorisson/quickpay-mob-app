...
function backgroundImagesForApp(shellPath, manifest, isDetached, sdkVersion) {
  ...
  if (url) {
    ...
    const drawableDirectory = parseSdkMajorVersion(sdkVersion) >= 33 ? 'drawable-mdpi' : 'drawable-xxxhdpi';
    return [{
      url,
      path: _path.default.join(basePath, drawableDirectory, 'shell_launch_background_image.png')
    }];
  }
...
function shouldShowLoadingView(manifest, sdkVersion) {
  ...
  return resizeMode && (parseSdkMajorVersion(sdkVersion) >= 33 ? resizeMode === 'contain' : resizeMode === 'cover');
}
...
async function runShellAppModificationsAsync(context, sdkVersion) {
  ...
  let backgroundImages = backgroundImagesForApp(shellPath, manifest, isRunningInUserContext, sdkVersion);
...
async function buildShellAppAsync(context, sdkVersion, buildType) {
  let shellPath = shellPathForContext(context);
  const ext = buildType === 'app-bundle' ? 'aab' : 'apk';
  const isRelease = !!context.build.android; // concat on those strings is not very readable, but only alternative here is huge if statement
  ...
  if (isRelease) {
    ...
    await _fsExtra.default.copy(outputPath, `/tmp/shell-debug.${ext}`);
  }
}
