...
async function spawnAsyncThrowError(...args) {
  if (args.length === 2) {
    return (0, _spawnAsync.default)(args[0], args[1], {
      stdio: 'inherit',
      cwd: process.cwd()
    });
  } else {
    ...
    const promise = (0, _spawnAsync.default)(...args);

    if (pipeToLogger && promise.child) {
      const streamsKeys = _lodash.default.isObject(pipeToLogger) ? _lodash.default.keys(_lodash.default.pickBy(pipeToLogger, _lodash.default.identity)) : ['stdout', 'stderr'];

      const streamsToLogs = _lodash.default.pick(promise.child, streamsKeys);

      (0, _Logger.pipeOutputToLogger)(streamsToLogs, options.loggerFields, options);
    }
...
function rimrafDontThrow(directory) {
  try {
    ...
    _Logger.default.warn(`There was an issue cleaning up, but your project should still work. You may need to manually remove ${directory}. (${e})`);
  }
}
