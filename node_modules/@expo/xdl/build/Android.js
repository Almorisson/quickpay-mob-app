...
async function _downloadApkAsync() {
  ...
  if (await _fsExtra.default.exists(apkPath)) {
    return apkPath;
  }

  await _Api.default.downloadAsync(versions.androidUrl, _path.default.join(_apkCacheDirectory(), `Exponent-${versions.androidVersion}.apk`));
  return apkPath;
}
...
async function checkSplashScreenImages(projectDir) {
  ...
  const splashScreenMode = _lodash.default.get('android.splash.resizeMode') || _lodash.default.get(exp, 'splash.resizeMode');
  ...
  for (const {
    ...
    if (imageRelativePath) {
      ...
      if (!splashImage) {
        _Logger.default.global.warn(`Couldn't read dimensions of provided splash image '${_chalk.default.italic(splashImage)}'. Does the file exist?`);
  ...
  if (androidSplashImages.length === 0) {
    _Logger.default.global.warn(`Splash resizeMode is set to 'cover', but you haven't provided any images for different DPIs.
...
See https://docs.expo.io/versions/latest/guides/splash-screens/#differences-between-environments---android for more information`);
  ...
  if (_lodash.default.some(androidSplashImages, ({
    ...
    _Logger.default.global.warn(`Splash resizeMode is set to 'cover' and you've provided different images for different DPIs,
