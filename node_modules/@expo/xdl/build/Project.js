...
async function exportForAppHosting(projectRoot, publicUrl, assetUrl, outputDir, options = {}) {
  ...
  await _writeArtifactSafelyAsync(projectRoot, null, _path.default.join(outputDir, 'android-index.json'), JSON.stringify(exp)); // save the ios manifest
...
async function optimizeAsync(projectRoot = './', options = {}) {
  ...
  const {
    assetJson,
    assetInfo
  } = await readAssetJsonAsync(projectRoot); // Keep track of which hash values in assets.json are no longer in use
  ...
  const {
    allFiles,
    selectedFiles
  } = await getAssetFilesAsync(projectRoot, options);
  ...
  for (const image of images) {
    ...
    if (amountSaved < 0) {
      ...
      _fsExtra.default.renameSync(newName, image);

      assetInfo[hash] = true;

      _Logger.default.global.info(_chalk.default.gray(`Compressed version of ${image} was larger than original. Using original instead.`));
    ...
    if (amountSaved) {
      ...
      _Logger.default.global.info(`Saved ${toReadableValue(amountSaved)}`);
    } else {
  ...
  if (totalSaved === 0) {
    ...
    _Logger.default.global.info(`Finished compressing assets. ${_chalk.default.green(toReadableValue(totalSaved))} saved.`);
  }
