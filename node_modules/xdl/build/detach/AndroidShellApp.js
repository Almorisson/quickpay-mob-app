...
let copyInitialShellAppFilesAsync = exports.copyInitialShellAppFilesAsync = (() => {
  var _ref2 = _asyncToGenerator(function* (androidSrcPath, shellPath, isDetached = false) {
    if (androidSrcPath && !isDetached) {
      ...
      yield spawnAsyncThrowError('../../tools-public/check-dynamic-macros-android.sh', [], {
        pipeToLogger: true,
    ...
    if (!isDetached) {
      ...
      yield copyToShellApp('versioned-abis');
...
let runShellAppModificationsAsync = exports.runShellAppModificationsAsync = (() => {
  var _ref5 = _asyncToGenerator(function* (context, isDetached = false) {
    ...
    if (isDetached) {
      ...
      yield deleteLinesInFileAsync('WHEN_DISTRIBUTING_REMOVE_FROM_HERE', 'WHEN_DISTRIBUTING_REMOVE_TO_HERE', appBuildGradle);
...
let removeObsoleteSdks = (() => {
  var _ref9 = _asyncToGenerator(function* (shellPath, requiredSdkVersion) {
    ...
    if (effectiveSdkVersion === 'UNVERSIONED') {
      yield regexFileAsync('TEMPORARY_ABI_VERSION = null;', `TEMPORARY_ABI_VERSION = "${requiredSdkVersion}";`, filePathsToTransform.constants);
    }

    yield Promise.all(Object.values(filePathsToTransform).map(function (filePath) {
      return removeInvalidSdkLinesWhenPreparingShell(effectiveSdkVersion, filePath);
    }));
  });

  return function removeObsoleteSdks(_x11, _x12) {
...
exports.createAndroidShellAppAsync = (() => {
  var _ref4 = _asyncToGenerator(function* (args) {
    ...
    yield removeObsoleteSdks(shellPath, sdkVersion);
 ...
 */
const removeInvalidSdkLinesWhenPreparingShell = (() => {
  var _ref8 = _asyncToGenerator(function* (majorSdkVersion, filePath) {
    yield regexFileAsync(new RegExp(`BEGIN_SDK_${majorSdkVersion}`, 'g'), `WHEN_PREPARING_SHELL_REMOVE_TO_HERE`, filePath);
    yield regexFileAsync(new RegExp(`END_SDK_${majorSdkVersion}`, 'g'), `WHEN_PREPARING_SHELL_REMOVE_FROM_HERE`, filePath);
    yield deleteLinesInFileAsync(/WHEN_PREPARING_SHELL_REMOVE_FROM_HERE/g, 'WHEN_PREPARING_SHELL_REMOVE_TO_HERE', filePath);
  });

  return function removeInvalidSdkLinesWhenPreparingShell(_x9, _x10) {
