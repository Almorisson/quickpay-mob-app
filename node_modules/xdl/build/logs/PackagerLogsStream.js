...
class PackagerLogsStream {
  ...
  _handleMetroEvent(originalChunk) {
    const chunk = _extends({}, originalChunk);
    let { msg } = chunk;
    ...
    switch (msg.type) {
      ...
      case 'bundling_error':
        chunk.msg = this._formatModuleResolutionError(msg.error) || this._formatBundlingError(msg.error);
        chunk.level = (_Logger || _load_Logger()).default.ERROR;
        break;
      ...
      default:
        ...
        chunk.level = (_Logger || _load_Logger()).default.DEBUG;
        break;
    }
    this._enqueueAppendLogChunk(chunk);
  }
  ...
  _handleUpdateBundleTransformProgress(progressChunk) {
    ...
    if (bundleError) {
      ...
      progressChunk.level = (_Logger || _load_Logger()).default.ERROR;
      if (msg.error) {
    ...
    if (this._onProgressBuildBundle) {
      ...
      if (bundleComplete) {
        this._onFinishBuildBundle && this._onFinishBuildBundle(bundleError, this._bundleBuildStart, bundleBuildEnd, progressChunk);
  ...
  _formatModuleResolutionError(error) {
    ...
    let relativePath = _path.default.relative(this._projectRoot, originModulePath);
  ...
  _formatBundlingError(error) {
    ...
    if (message && error.filename) {
      let escapedFilename = (0, (_escapeStringRegexp || _load_escapeStringRegexp()).default)(error.filename);
      message = message.replace(new RegExp(`Error in ${escapedFilename}: ${escapedFilename}:`), `Error in ${error.filename}:`);
    }
